...
no kernel mount, no FUSE, no loopback device, and no C library. It is written in
safe Rust (`#![forbid(unsafe_code)]`) and runs anywhere Rust runs.
...
the inodes awaiting deletion are recorded in an orphan file (`orphan_file`) rather
than a list threaded through the superblock. Files map their blocks with extent
trees of any depth, and a directory that outgrows one block gains a hash index
(`dir_index`).

It is at once:

- **a Rust library** you link, not a process you shell out to;
- **rootless and kernel-free** — no mount, no loopback, no FUSE, no privileged
  syscall;
- **cross-platform** — pure Rust with no kernel dependency, so it builds and
  reads ext2/3/4 images on Linux, macOS, Windows, and the BSDs;
- **deterministic** — the default output is byte-reproducible: the filesystem
  UUID and timestamps are inputs, never read from the clock or a random source,
  and names hash the same way whatever machine builds the image;
